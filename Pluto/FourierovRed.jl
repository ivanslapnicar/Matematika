...
using SymPyPythonCall, Plots, PlutoUI, QuadGK
...
begin
	K=10
	S=a(0)/2+sum([a(n)*cos(2*PI*n*x/T)+b(n)*sin(2*PI*n*x/T) for n=1:10])
	g(x)=S(x)
...
c=sympy.fourier_series(x^2,(x,x₀,x₁)).truncate(K)
...
begin
	K₁=20
	c₁=sympy.fourier_series(x,(x,x₀,x₁)).truncate(K₁)
	cs₁=[c₁(x) for x in X]
	title₁="Funkcija i "*string(K₁)*" članova reda"
...
function Fourier(f::Function, n::Int, x₀, x₁)
...
manifest_format = "2.0"
project_hash = "8d4467267899f7458e8a04a740b731efb6d09c5d"
